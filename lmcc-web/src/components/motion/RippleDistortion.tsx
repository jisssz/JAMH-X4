...
// Default dark, premium, offline-ready LMCC background texture (rich slate-950 with deep emerald & teal contours)
export const DEFAULT_LMCC_TEXTURE = `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="900" viewBox="0 0 1600 900"><defs><radialGradient id="g1" cx="25%" cy="30%" r="60%"><stop offset="0%" stop-color="%23064e3b"/><stop offset="50%" stop-color="%230f2b48"/><stop offset="100%" stop-color="%23020617"/></radialGradient><radialGradient id="g2" cx="80%" cy="70%" r="50%"><stop offset="0%" stop-color="%230d9488" stop-opacity="0.35"/><stop offset="60%" stop-color="%23042f2e" stop-opacity="0.2"/><stop offset="100%" stop-color="%23020617" stop-opacity="0"/></radialGradient><pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse"><path d="M 40 0 L 0 0 0 40" fill="none" stroke="%23334155" stroke-width="0.75" stroke-opacity="0.2"/></pattern></defs><rect width="1600" height="900" fill="url(%23g1)"/><rect width="1600" height="900" fill="url(%23g2)"/><rect width="1600" height="900" fill="url(%23grid)"/></svg>`;
...
export const RippleDistortion: React.FC<RippleDistortionProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[RippleDistortion] WebGL not supported; using fallback.', e);
      return;
    }
    ...
    // Initial ambient ripple on load for subtle visual feedback
    if (!reduceMotion) {
      setTimeout(() => {
        if (!disposed && mount) {
          setNewWave(width * 0.5, height * 0.5, 1.2);
        }
      }, 300);
    }
  ...
  return (
    <div
      ref={mountRef}
      className={`ripple-distortion-container relative w-full h-full overflow-hidden bg-slate-950 ${className}`.trim()}
      style={style}
    />
  );
};
